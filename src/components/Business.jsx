...
const FeatureCard = features => {
  const { icon, title, content } = features;
  return (
    <div
      className={`flex flex-row p-6  rounded-[20px]   feature-card max-w-[500px] content-baseline items-center `}
    >
      <div
        className={`w-[64px] h-[64px] rounded-full ${styles.flexCenter} bg-dimBlue`}
      >
        <img src={icon} alt="icon" className="w-[50%] h-[59%] object-contain" />
      </div>
